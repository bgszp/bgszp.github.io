...
// === 4. LOGIKA CATATAN PUBLIK 2 ===
window.tambahCatatanPublik2 = function() {
    const input = document.getElementById("inputCatatanPublik2");
    const teks = input.value.trim();
    if(!teks) return;

    const dbRef = ref(db, 'catatan_publik_2');
    push(dbRef, {
        pesan: teks,
        waktu: new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) + ' ' + new Date().toLocaleDateString('id-ID')
    }).then(() => {
        input.value = "";
        if(window.showToast) window.showToast("Catatan Publik 2 Terkirim! 🚀");
    });
};

window.hapusCatatanPublik2 = function(id) {
    const dbRef = ref(db, 'catatan_publik_2/' + id);
    remove(dbRef).then(() => {
        if(window.showToast) window.showToast("Catatan 2 dihapus! 🗑️");
    });
};

const dbRefPublik2 = ref(db, 'catatan_publik_2');
onValue(dbRefPublik2, (snapshot) => {
    const wadah = document.getElementById("wadahCatatanPublikList2");
    if (!wadah) return; 
    ...
    if (!data) {
        wadah.innerHTML = '<div style="color:#6e678a; font-size:12px; text-align:center; margin-top:20px;">Belum ada catatan publik 2. Jadilah yang pertama menulis!</div>';
        return;
    }

    let copiedKeys = JSON.parse(localStorage.getItem('copied_publik_2_keys') || '[]');
    const reversedKeys = Object.keys(data).reverse();
    ...
    reversedKeys.forEach((key, index) => {
        ...
        row.innerHTML = `
            <div style="font-size:12px; color:#00b4db; width:20px; font-weight:bold; margin-top: 5px;">${index + 1}.</div>
            <div style="flex: 1; display: flex; flex-direction: column; gap: 4px; z-index: 2;">
                <div class="teks-publik" style="font-size: 13px; color: #fff; word-break: break-all; white-space: pre-wrap; cursor: pointer;" title="Klik untuk menyalin">${item.pesan} <i class="fa-regular fa-copy copy-hint-icon" style="color: #00b4db; opacity: 1; margin-left: 5px;"></i></div>
                <div style="font-size: 10px; color: #8e85b3;"><i class="fa-regular fa-clock"></i> ${item.waktu || ''}</div>
            </div>
            <div style="display: flex; gap: 8px; margin-top: -2px;">
                <button class="btn-edit-publik btn-hapus-copas" style="color: #f0ad4e; font-size: 14px;" title="Edit"><i class="fa-solid fa-pen"></i></button>
        ...
        row.querySelector('.teks-publik').addEventListener('click', () => {
            if(window.eksekusiSalinTeks) {
                window.eksekusiSalinTeks(item.pesan).then(() => {
                    if(window.showToast) window.showToast("Catatan Publik 2 Disalin! ✨");
                    row.classList.remove("publik-nyala");
                    let currentKeys = JSON.parse(localStorage.getItem('copied_publik_2_keys') || '[]');
                    if (!currentKeys.includes(key)) {
                        currentKeys.push(key);
                        localStorage.setItem('copied_publik_2_keys', JSON.stringify(currentKeys));
                    }
                });
            }
        });
        ...
        row.querySelector('.btn-edit-publik').addEventListener('click', () => {
            const pesanBaru = prompt("Edit catatan publik 2:", item.pesan);
            if (pesanBaru !== null && pesanBaru.trim() !== "" && pesanBaru !== item.pesan) {
                const catatanRef = ref(db, 'catatan_publik_2/' + key);
                update(catatanRef, {
                    pesan: pesanBaru.trim(),
                    waktu: new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) + ' ' + new Date().toLocaleDateString('id-ID') + ' (Diedit)'
                }).then(() => {
        ...
        row.querySelector('.btn-hapus-publik').addEventListener('click', () => {
            window.hapusCatatanPublik2(key);
        });
